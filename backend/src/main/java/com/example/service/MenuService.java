...
public class MenuService {
    ...
    public List<Map<String, Object>> getAllMenus() {
        List<Menu> topMenus = menuRepository.findByParentIsNullOrderBySortOrderAsc();
        return topMenus.stream().map(this::menuToMap).collect(Collectors.toList());
    }
    ...
    public Map<String, Object> createTopMenu(String name, Integer sortOrder) {
        ...
        if (menuRepository.existsByNameAndParentIsNull(name)) {
            result.put("success", false);
     ...
     */
    @Transactional
    public Map<String, Object> createSubMenu(Long parentId, String name, Integer sortOrder) {
        Map<String, Object> result = new HashMap<>();
        ...
        if (menuRepository.existsByNameAndParentId(name, parentId)) {
            result.put("success", false);
     ...
     */
    @Transactional
    public Map<String, Object> updateMenu(Long id, String name, Integer sortOrder, Boolean isActive) {
        Map<String, Object> result = new HashMap<>();
        ...
        if (name != null && !name.trim().isEmpty()) {
            ...
            if (menu.getParent() == null) {
                if (menuRepository.existsByNameAndParentIsNull(name) && !name.equals(menu.getName())) {
                    result.put("success", false);
                    result.put("message", "顶级菜单名称已存在");
                    return result;
                }
            } else {
                if (menuRepository.existsByNameAndParentId(name, menu.getParent().getId()) && !name.equals(menu.getName())) {
                    result.put("success", false);
                    result.put("message", "该父菜单下已存在同名子菜单");
                    return result;
                }
